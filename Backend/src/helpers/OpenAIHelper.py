...
class AIHelper:
    ...
    def genrate_from_prompt(self, model, prompt, temperature=0, n=1) -> str:
        ...
        self.__loggerObj.info(
            f"Response Created: {str(response.choices[0].message.content)}"
        )
        if response.usage:
            self.__loggerObj.critical(f"Total Token {response.usage.total_tokens}")
        if response.choices[0].message.content:
            return response.choices[0].message.content
        else:
            return ""
        # except Exception as e:
        #     self.__loggerObj.critical("Not able to genrate answer")
        #     self.__loggerObj.exception(e)
        #     return ""